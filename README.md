...
- 病树点 3D 模型：病树点以 3D 松树模型显示，贴近真实场景
...
- 路径规划：病树巡查 / 路线规划
- 用户登录认证：账号密码登录，JWT 令牌认证，路由守卫自动拦截未登录访问
- 用户管理：用户列表、添加、编辑、删除
- 病树管理：属性筛选、空间搜索（经纬度 + 半径）、病树列表、删除
- 时空趋势分析：月度病树趋势图与时间滑块按月过滤，疫情重心迁移动态飞线与扩散圆环动画展示
- 3D Tiles 加载：加载 / 卸载香港区域 3D 山体模型
- 点击取坐标：点击地图拾取经纬度与高程，支持一键复制
